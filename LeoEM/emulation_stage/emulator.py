...
def update_precomputed_link(link_info_all_cycles, net):
    ...
    for index in range(len(link_info_all_cycles)):
        ...
        # Log the timestamp for the current cycle
        log_cycle_timestamp("cycle_time.log", index + 1)
        ...
        # end the emulation with desired cycle number
        if index == 300:
            break
...
def log_iperf_throughput(h1, log_file, duration):
    h1.cmd(f'iperf3 -c 10.0.1.102 -t {duration} > {log_file} &')

def start_tshark_capture(h1, interface, filter_expr, pcap_file, duration):
    # h1.cmd(f"sudo tshark -i {interface} -f \"{filter_expr}\" -w {pcap_file} -a duration:{duration} &")
    h1.cmd(f"sudo tshark -i {interface} -f \"{filter_expr}\" -w {pcap_file} &")

def log_handovers(log_file_path='handover.log'):
    print("Logging handover timestamp...")
    current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S.%f UTC')
    
    # Append the timestamp to the log file
    try:
        with open(log_file_path, 'a') as log_file:
            log_file.write(current_timestamp + '\n')
        print(f"Logged timestamp: {current_timestamp}")
    except Exception as e:
        print(f"Error: An error occurred while writing to the log file. {e}")

def log_cycle_timestamp(log_file, cycle):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(log_file, 'a') as f:
        f.write(f"Cycle {cycle}: {timestamp}\n")
        f.flush()
...
def main():
    ...
    # Setting up iperf servers and clients
    h1 = net.get('h1')
    h2 = net.get('h2')
    h2.cmd('iperf3 -s -&')  # Start iperf server on h2
    duration = 3 * 60 * 60 # Todo: Change to whatever duration you want

    throughput_file = "throughput_" + path_info_file
    log_iperf_throughput(h1, throughput_file, duration)

    # Start tshark capture for iperf traffic
    interface = '2'  # VM interface
    pcap_file = "iperf_traffic.pcap"
    filter_expr = "tcp and host 10.0.1.102 and port 5201"
    start_tshark_capture(h1, interface, filter_expr, pcap_file, duration)
